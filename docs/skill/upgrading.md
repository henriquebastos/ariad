...
Add repeatable `--installed-package TARGET_RELATIVE_PATH` for runtime-native installations outside conventional probes, and select another safe target-relative `--skill-destination` when needed. Without an explicit destination, installation remains a manual choice and is not classified safe-additive. Add `--json` for stable machine-readable output. The audit inventories Git state, instruction scopes and common harness files, the exact entrypoint contract, closed and digest-verified package manifests, candidate provenance and compatibility components, legacy/modular docs, references, and adjacent project memory. Manifest paths and repository scans do not follow symlinks. It writes no report or cache and changes no target files.
