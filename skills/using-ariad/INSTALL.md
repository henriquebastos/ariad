...
The Ariad source repository intentionally does not contain a generated `references/` copy. The installed copy is a pinned, self-contained snapshot. Record the actual project-relative installed `SKILL.md` path, source repository, and resolved commit or immutable release identifier in the target project's chosen dependency or decision record.
...
Inspect the target repository and propose the smallest useful project memory surface. Copy only absent files that fit the project. Integrate the stable `docs/ariad/index.md` router into existing agent instructions rather than replacing them. When a destination already exists, preserve it and propose a semantic reconciliation for Navigator review.
...
- runtime discovery or the documented project-owned manual-loading path reaches `SKILL.md`;
