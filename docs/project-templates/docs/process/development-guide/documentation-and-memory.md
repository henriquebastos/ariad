...
Promote durable meaning during normal lifecycle closure. When ending, pausing, compacting, interrupting, or handing off a session, follow the installed skill's canonical `references/method/memory-closure.md` protocol. Fold it into an existing closure surface when they coincide; do not emit a second receipt. Promote meaning once into its smallest owner, report dirty state truthfully, and do not preserve transcripts or create generic session-summary files by default. Record a worklog entry only for a meaningful milestone.
